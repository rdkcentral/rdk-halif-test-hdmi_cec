...
| | | Transmit a CEC Command (as per 1.4b HDMI CEC spec) to get the CEC Version for a logical address that doesn't exist.| HdmiCecAddLogicalAddress HdmiCecTx HdmiCecRemoveLogicalAddress | Y | NA  |  Control plane can unplug or switch off a previously existing CEC device |
