...
void test_hdmicec_hal_l1_open_negative( void )
{
    ...
    if (HDMI_CEC_IO_INVALID_HANDLE  != result) { HDMICEC_ASSERT_AUTO_TERM(); UT_FAIL  ("Check failed"); }

    result = HdmiCecOpen( &handle );
    if (HDMI_CEC_IO_SUCCESS != result) { HDMICEC_ASSERT_AUTO_TERM(); UT_FAIL  ("Check failed"); }
    
    result = HdmiCecOpen( &handle );
    if (HDMI_CEC_IO_ALREADY_OPEN != result) { HDMICEC_ASSERT_AUTO_TERM(); UT_FAIL  ("Check failed"); }
...
void test_hdmicec_hal_l1_close_negative( void )
{
    ...
    UT_LOG("\n In %s [%02d%03d]\n", __FUNCTION__, gTestGroup, gTestID);
    result = HdmiCecClose( handle );
    if (HDMI_CEC_IO_NOT_OPENED  != result) { HDMICEC_ASSERT_AUTO_TERM(); UT_FAIL  ("Check failed"); }
...
void test_hdmicec_hal_l1_getPhysicalAddress_negative( void )
{
    ...
    if (HDMI_CEC_IO_INVALID_ARGUMENT  != result) { HDMICEC_ASSERT_AUTO_TERM(); UT_FAIL  ("Check failed"); }
    ...
    if (physicalAddress>maxVal) {
        HDMICEC_ASSERT_AUTO_TERM(); UT_FAIL  ("Check failed");
    }
...
void test_hdmicec_hal_l1_removeLogicalAddress_sinkDevice_negative( void )
{
    ...
    result = HdmiCecRemoveLogicalAddress( handle, logicalAddress );
    if (HDMI_CEC_IO_ALREADY_REMOVED != result) { HDMICEC_ASSERT_AUTO_TERM(); UT_FAIL  ("Check failed"); }
...
void test_hdmicec_hal_l1_hdmiCecTx_sinkDevice_negative( void )
{
    ...
    result = HdmiCecTx(handle, buf, len, &ret);
    if (HDMI_CEC_IO_SENT_FAILED != result) { HDMICEC_ASSERT_AUTO_TERM(); UT_FAIL  ("Check failed"); }
...
void test_hdmicec_hal_l1_portDisconnected_sink( void )
{
    ...
    if (HDMI_CEC_IO_SUCCESS != result) { HDMICEC_ASSERT_AUTO_TERM(); UT_FAIL  ("Check failed"); }
    if (HDMI_CEC_IO_SENT_BUT_NOT_ACKD != ret) { HDMICEC_ASSERT_AUTO_TERM(); UT_FAIL  ("Check failed"); }
